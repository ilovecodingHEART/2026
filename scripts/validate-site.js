...
async function main(){
  ...
  for (const file of pages) {
    routes.add('/' + encodeURIComponent(file));
    const html = fs.readFileSync(path.join(ROOT,'archive','pages',file),'utf8');
    if (forbidden.test(html)) throw new Error(`${file} contains forbidden remote URL`);
    if (archiveMarkers.test(html)) throw new Error(`${file} contains archive artifact`);
    for (const a of attrs(html)) {
      if (forbidden.test(a.value)) throw new Error(`${file} forbidden ${a.attr}: ${a.value}`);
  ...
  await ok('/api/games'); await ok('/api/catalog'); await ok('/api/local-compat',{method:'POST'});
  console.log(`validated ${pages.length} pages and ${routes.size} local routes`);
}
